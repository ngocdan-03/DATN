...
import { TRANSACTION_STATUS, TYPE_OPTIONS } from "../../../constants/finance";


const DEFAULT_FILTERS = { type: "", status: "" };

export default function AdminTransactionsFilter({ value = DEFAULT_FILTERS, onApply }) {
      ...
      {isOpen && (
        <div className="absolute right-0 top-12 z-40 w-[420px] max-w-[85vw] overflow-hidden rounded-2xl border border-[#c4c6cd]/40 bg-white shadow-[0_24px_60px_-30px_rgba(4,22,39,0.5)]">
          <div className="bg-[linear-gradient(135deg,#041627_0%,#123456_100%)] px-5 py-4 text-white">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-1 text-lg font-black [font-family:Manrope]">Bộ lọc giao dịch</h3>
              </div>
              <button
                type="button"
          ...
          <div className="space-y-4 p-5">
            <div>
              <label className="mb-2 inline-flex text-[11px] font-bold uppercase tracking-wider text-[#5e6570]">
                Loại giao dịch
              </label>
              <select
            ...
            <div>
              ...
              >
                ...
                {Object.values(TRANSACTION_STATUS).map((opt) => (
                  <option key={opt.value} value={opt.value}>{opt.label}</option>
                ))}
              </select>
            </div>
