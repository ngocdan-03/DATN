...
export default function SendOtpVerify() {
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      const result = await authService.sendOtpVerify({ email });
      ...
      setModal({
        ...
        type: "success",
        title: "Đã gửi OTP",
        message: result?.message || "Mã OTP đã được gửi về email của bạn.",
        confirmText: "Tiếp tục xác thực",
      });
    } catch (err) {
      setNextEmail("");
      setModal({
  ...
  return (
    <section className="px-6 py-12">
      <div className="mb-8">
        ...
        <h2 className="mb-3 text-4xl font-bold text-slate-900">Gửi OTP xác thực</h2>
